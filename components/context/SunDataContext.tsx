...
export const SunDataProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const cacheRef = useRef<SunDataCache>({});
  const countyCacheRef = useRef<Map<string, string>>(new Map());
  const pendingRequestsRef = useRef<Map<string, Promise<SunData | null>>>(
    new Map()
  );
  ...
  const getCounty = useCallback(
    async (beachId: string): Promise<string | null> => {
  ...
  const getSunData = useCallback(
    async (beachId: string, date: Date): Promise<SunData | null> => {
  ...
  const prefetchSunData = useCallback(
    async (beachId: string, dates: Date[]) => {
  ...
  return (
    <SunDataContext.Provider value={value}>{children}</SunDataContext.Provider>
  );
};
