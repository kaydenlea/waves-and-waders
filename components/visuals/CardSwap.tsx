...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  ({ customClass, ...rest }, ref) => (
    <div
      ref={ref}
      {...rest}
      className={`w-[500px] h-[400px] absolute top-1/2 left-1/2 rounded-xl border border-white bg-highlight-5 p-2 [transform-style:preserve-3d] [will-change:transform] [backface-visibility:hidden] ${
        customClass ?? ""
      } ${rest.className ?? ""}`.trim()}
    />
  )
...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  return (
    <div
      ...
      style={{ width, height }}
    >
      <div className="w-full mx-auto relative xl:static">{rendered}</div>
    </div>
  );
};
