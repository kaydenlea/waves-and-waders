...
const items = [
  { node: <Zap className="w-4 h-4" />, title: "Live conditions" },
  {
    node: <ShieldCheck className="w-4 h-4" />,
    title: "Verified data sources",
  },
  { node: <Trophy className="w-4 h-4" />, title: "Personalized forecasts" },
];
      ...
      (item: LogoItem, key: React.Key) => {
        ...
        if (isNodeItem) {
          content = (
            <span
              className={cx(
                "space-x-2 rounded-full border border-border bg-background/20 shadow-sm px-3 py-2 backdrop-blur-md",
          ...
          itemAriaLabel = item.ariaLabel ?? item.title ?? undefined;
        } else {
          content = (
            <img
              className={cx(
          ...
          itemAriaLabel = item.alt ?? item.title ?? undefined;
        }
